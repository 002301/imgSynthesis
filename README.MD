

### 1.0.5 版本
修改set函数为init
添加addTxt()方法，可以添加一个文本；
添加addImg()方法，可以添加一个图片；
添加getImg()方法，可以得到合成的图片；


### 1.0.4 版本

添加文本竖向排版功能
txt参数添加 writingMode 属性 传入参数 'td' 文字就会已竖排形式排版
例如
txt':[
{x:558,y:755,text:rd,`writingMode:'td'`,color:'red',size:'18px'},
]

 # canvas图片合并类
// 传入参数只有src是必填，其他都可以不填，定位默认0，0

```js
  @width:Number //canvas宽度
  @height:Number //canvas高度
  @txt:Array //合成文本
      @text:String 合成内容
      @x:Number //x轴
      @y:Number //y轴
      @width:Number //文本的宽度
      @size:24 文本字号
      @color:'#000' //颜色
      @writingMode //writingMode:'td' //文字竖排显示，默认横排
  @img:Array //合并图片数组
      @src:String //图片地址(必填)
      @x:Number //图片x轴
      @y:Number //图片y轴
      @width:Number //图片宽度
      @height:Number //~~图片高度~~
```

#### 参数示例
```js
例1：
  imgSynthesis.init({width:100,height:100,'txt':[{text:'123',x:100,y:100,width:123,size:'60px'},{text:'123',x:100,y:100}],'img':[{x:0,y0,src:'123.jpg'},{x:0,y0,src:'123.jpg'}]}).then((e)=>{
    let img = e;
  });
例2：
imgSynthesis.init({
    'msg':{x:300,y:400,text:this.names},
    'img':[{src:imgAssets[bgr]},{y:656,src:er}]
    }).then((e)=>{
        this.resultBase = e;
    })
// 1.0.5 添加功能
imgSynthesis.addTxt({text:'123',x:100,y:100}).then(()=>{
    //添加完成
    let img = imgSynthesis.getImg();
});
imgSynthesis.addImg({{x:0,y0,src:'123.jpg'}).then(()=>{
    let img = imgSynthesis.getImg();
});

```

#### vue 框架

> npm i img-synthesis
```js
//vue 框架示例
<template>
    <div class="app">
        <img :src="resultBase" class="result-img">
    </div>
</template>

<script>
    import imgSynthesis from 'img-synthesis';
    export default {
        data(){
            return{
                names:'1234',
                resultBase:null,
            }
        },
        mounted(){
            let ls =  new imgSynthesis();
            ls.init({
            'txt':{x:300,y:400,text:this.names},
            'img':[{src:img}]
            }).then((e)=>{
                this.resultBase = e;
            })
        },
    };
   let img ='data:image/jpeg;base64,'
</script>

<style lang="postcss" scoped>
.result-img{
	width:640px;
	height:1236px;
	position: absolute;
	z-index: 2;
}
</style>
```
